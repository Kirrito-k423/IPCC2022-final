...
int main(int argc, const char * argv[]) {
    ...
    int start_off_tree_num, end_off_tree_num, i;
    int while_times = 0;
    printTime("before while \t\t took %f ms\n");
    while(1){
        // 从similarity里确定分配的任务列表
        start_off_tree_num = while_times * task_pool_size;
        end_off_tree_num = start_off_tree_num + task_pool_size;
        ...
        for(i=start_off_tree_num; i<end_off_tree_num; i++){
            int * thread_similarity_tree_address = similarity_tree_list + (i-start_off_tree_num) *(similarity_tree_length);
            memset(thread_similarity_tree_address, 0 , sizeof(int)* similarity_tree_length);

            if(i>similarity_tree_length || similarity_tree[i]==1){
                continue;
            }
            ...
            adjust_similarity_tree(i, bfs_process1, bfs_process2, thread_similarity_tree_address, copy_off_tree_edge);
